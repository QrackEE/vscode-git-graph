...
describe('GitGraphView', () => {
	...
	describe('WebviewPanel Construction', () => {
		it('Should construct a new WebviewPanel', () => {
			...
			// Assert
			expect(vscode.window.createWebviewPanel).toHaveBeenCalledWith('git-graph', 'Git History', vscode.ViewColumn.Two, {
				enableScripts: true,
				localResourceRoots: [vscode.Uri.file(path.join('/path/to/extension', 'media'))],
				retainContextWhenHidden: true
			});
			expect(spyOnLog).toHaveBeenCalledWith('Created Git History View');
		});
		...
		it('Should construct a new WebviewPanel, providing loadViewTo', () => {
			...
			// Assert
			const mockedWebviewPanel = vscode.getMockedWebviewPanel(0);
			expect(spyOnLog).toHaveBeenCalledWith('Created Git History View (active repo: /path/to/repo)');
		...
		it('Should construct a new WebviewPanel (no active text editor)', () => {
			...
			// Assert
			expect(vscode.window.createWebviewPanel).toHaveBeenCalledWith('git-graph', 'Git History', vscode.ViewColumn.One, {
				enableScripts: true,
				localResourceRoots: [vscode.Uri.file(path.join('/path/to/extension', 'media'))],
				retainContextWhenHidden: true
			});
	...
	describe('sendMessage', () => {
		...
		it('Should log an error message when Webview.postMessage rejects, and the GitGraphView hasn\'t been disposed', async () => {
			...
			await waitForExpect(() => {
				...
				expect(spyOnLog).not.toHaveBeenCalled();
				expect(spyOnLogError).toHaveBeenCalledWith('Unable to send "viewScm" message to the Git History View.');
			});
		});
		...
		it('Should log an information message when Webview.postMessage rejects, and the GitGraphView has been disposed', async () => {
			...
			await waitForExpect(() => {
				...
				expect(spyOnLog).toHaveBeenCalledWith('The Git History View was disposed while sending "viewScm" message.');
				expect(spyOnLogError).not.toHaveBeenCalled();
			});
		});
		...
		it('Shouldn\'t send a message to the Webview if it has been disposed', async () => {
			...
			await waitForExpect(() => {
				...
				expect(spyOnLog).toHaveBeenCalledWith('The Git History View has already been disposed, ignored sending "viewScm" message.');
				expect(spyOnLogError).not.toHaveBeenCalled();
			});
		});
	});
	...
	describe('getHtmlForWebview', () => {
		...
		afterEach(() => {
			...
			expect(mockedWebviewPanel.panel.webview.html).toContain('<title>Git History</title>');
		...
		it('Should get HTML when no Git executable is known', () => {
			...
			// Assert
			const mockedWebviewPanel = vscode.getMockedWebviewPanel(0);
			expect(mockedWebviewPanel.panel.webview.html).toContain('<h2>Unable to load Git History</h2>');
		...
		it('Should get HTML when no repositories are found in the workspace', () => {
			...
			const mockedWebviewPanel = vscode.getMockedWebviewPanel(0);
			expect(mockedWebviewPanel.panel.webview.html).toContain('<h2>Unable to load Git History</h2>');
			expect(mockedWebviewPanel.panel.webview.html).toContain('No Git repositories were found in the current workspace when it was last scanned by Git History.');
